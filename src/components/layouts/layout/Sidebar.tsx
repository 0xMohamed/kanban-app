...
export default function Sidebar() {
	...
	const boards = [
		{ title: 'refactoring', id: 4299 },
		{ title: 'marketing', id: 1221 },
		{ title: 'roadmap', id: 24153 },
	];
	...
	return (
		<aside
			className={`fixed left-2 top-2 flex h-[calc(100vh-1rem)] w-56 flex-col items-center justify-between rounded-lg px-2 py-4 pb-6 text-light-3 ${
				isDarkMode ? 'bg-dark-2' : 'bg-light-2'
			}`}
		>
			<h2 className='text-md mb-4 mt-1.5 flex select-none items-center gap-x-0.5 uppercase text-purple'>
				<span>Kan</span>
				<span className='bg h-2 w-2 rounded-full bg-purple'></span>
				<span>Ban</span>
			</h2>
						...
						{boards.map((board) => {
							return (
								<Link
									to={'/board/' + board.id}
									key={board.id}
									className={`flex w-full items-center gap-x-2 rounded-md px-4 py-2 capitalize transition-colors duration-200 ${
										parseInt(boardId!) === board.id ? 'bg-purple text-white' : ''
									}`}
								>
									<PiKanban />
									<h4>{board.title}</h4>
